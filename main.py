...
async def run_cli_strategy(budget: float, currency: str, calorie_target: float, goal: str, preferences: str):
    ...
    if strategy:
        ...
        print("==================================================")
        print(f"Weekly Total Cost: {data_dict.get('weekly_total_cost')} {data_dict.get('currency')}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Wealth-Health Strategist AI Agent CLI Runner")
    parser.add_argument("--budget", type=float, default=200.0, help="Weekly grocery budget")
    parser.add_argument("--currency", type=str, default="USD", choices=["USD", "INR"], help="Currency (USD/INR)")
    parser.add_argument("--calories", type=float, default=2000.0, help="Daily target calorie count")
    parser.add_argument("--goal", type=str, default="weight_loss", choices=["weight_loss", "muscle_gain", "maintenance"], help="Weight Goal")
    parser.add_argument("--preferences", type=str, default="High protein", help="Dietary preferences or constraints")
    
    args = parser.parse_args()
    
    asyncio.run(run_cli_strategy(
        budget=args.budget,
